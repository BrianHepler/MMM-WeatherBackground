    ...
    if (typeof this.config.externalCollections === "string") {
      fetch('modules/MMM-WeatherBackground/' + this.config.externalCollections)
        .then(async (response) => {
          ...
          this.collections = await Object.assign({}, this.config.collections, await response.json())
        })
        .catch((err) => {
          if (config.verbose) Log.err(`[WHTBGR] External Collection Error: ${this.config.externalCollections}`)
        })
    }
  },
...
//https://source.unsplash.com/collection/4733334/?winter,day,cloudy&s=1631542013371
  loadImage: function (target, {monthKeyword, description} = {}) {
    ...
    url += (matchedCollection) ? `/collection/${matchedCollection}${size}/?&` : `/featured/${size}?` + convertedKeywords.join(',') + "&"
    url += `s=${seed}`
    var drawImage = (dom) => {
